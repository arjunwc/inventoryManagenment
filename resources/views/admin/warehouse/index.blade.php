...
<div class="content-wrapper" style="min-height: 926px;">
  <section class="content-header">
    ...
    <a href="{{url('administrator/warehouse/add')}}" id="btn_add_new_data" class="btn btn-sm btn-success" title="Add Data">
...
function getList() {
        ...
        if(userType=="Admin"){
          ...
          var url=baseUrl + "warehouselist?title=" + title+'&per-page='+page+'&user_id='+user_id;
        }else{
          var url=baseUrl + "warehouselist?title=" + title+'&per-page='+page;
        }
        $.ajax({
            url: url,
            type: 'get',
    ...
    function fetch_date(page) {
        ...
        if(userType=="Admin"){
          var user_id = $('#user_id').val();
          var url=  baseUrl + "warehouselist?title=" + title + '&page=' + page+'&per-page='+pages+'&user_id='+user_id;
        
        }else{
          var url=baseUrl + "warehouselist?title=" + title + '&page=' + page+'&per-page='+pages;
        }
        $.ajax({
            url: url,
            type: 'get',
    ...
    function deleteRecord(id) {
      swal({
            ...
            closeOnConfirm: false
              ...
              $.get(baseUrl + "warehouse/delete/" + id, function(data, status) {
                if (data.success) {
...
function changeSatus(id, status) {
    $.ajax({
        url: baseUrl + "warehouse/change_status/" + id + "/" + status,
        type: 'get',
        dataType: 'json',
        beforeSend: function() {},
        complete: function() {},
