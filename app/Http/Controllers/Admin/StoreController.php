...
class StoreController extends Controller
{
    public function index()
    {
        ...
        $output['module_name'] = "Store Management";
        $output['module_page'] = "List of Store";
        return view('admin.store.index', $output);
    }
    ...
    public function get_list(Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            $html    = "";
            $records = Store::where($search)->orderBy('created_at', 'DESC')
                ->paginate($page_limit);
            $response['success']    = true;
            $response['html']       = view('admin.store.list', ['records' => $records])->render();
    ...
    public function get_record_by_id($id)
    {
        ...
        return view('admin.store.add', $output);
    }
    ...
    public function store(Request $request)
    {
        ...
        if ($request->isMethod('post')) {
            ...
            if (!$validator->fails()) {
                ...
                $insert_record    = Store::create($input);
                if ($insert_record->id) {
                    $response['success']     = true;
                    $response['message']     = "Recored Store SuccessFully";
                    $response['resetForm']   = true;
                    $response['redirectURL'] = url('administrator/store');
                } else {
    ...
    public function update(Request $request)
    {
        ...
        if ($request->isMethod('post')) {
            ...
            if (!$validator->fails()) {
                ...
                $affectedRows = Store::where('id', $update['id'])->update($update);
                if ($affectedRows) {
                    $response['success']     = true;
    ...
    public function change_status($id, $status)
    {
        ...
        $user                = Store::where('id', $id)->update($update);
        $response['success'] = true;
    ...
    {
        try {
            $affectedRows = Store::where('id', $id)->delete();
            if ($affectedRows) {
                $response['success'] = true;
            } else {
                $response['success'] = false;
